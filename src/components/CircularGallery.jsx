...
class App {
  ...
  onTouchUp(e) {
    ...
    if (this.start !== undefined && e) {
      ...
      if (Math.abs(x - this.start) < 5 && this.onItemClick) {
        let width = 1;
        if (this.medias && this.medias[0]) {
             width = this.medias[0].width;
        }
        const itemIndex = Math.round(Math.abs(this.scroll.target) / width);
        if (this.items && this.items.length) {
            const index = itemIndex % this.items.length;
  ...
  addEventListeners() {
    ...
    // Bind touch/mouse events to canvas container for accurate clicking
...
export default function CircularGallery({
  ...
  textColor = '#ffffff',
  borderRadius = 0.05,
  font = 'bold 30px var(--font-display)',
  scrollSpeed = 2,
  scrollEase = 0.05,
  onItemClick
}) {
